...
$sql = "SELECT First_Name, Surname, Address, Date_of_Birth, Customer.Customer_Number, Balance, Current_Account.Current_Account_Number, Transaction.TransactionID, Date, Type, Amount
FROM Customer
...
echo "<br><select name='listbox4' id='listbox4' onclick='populateC()'>";
...
while ($row = mysqli_fetch_array($result))		// fetch array fetches a result row as an associative array
		{	
			...
			$type = $row['Type'];
			$amount = $row['Amount'];
			$allText = "$fname,$sname,$addrs,$dob,$num,$bal,$accNo,$tID,$date,$type,$amount";
